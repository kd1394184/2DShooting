...
public:
	...
	//文字表示（大きさ変更可能）
	void DrawString_Size(float _x, float _y, const char _text[], const Math::Vector4& _color, float fontRatio)
	{
		//ワイド文字列に変換する必要がある
		WCHAR* _wtext = new WCHAR[strlen(_text) + 1];
		mbstowcs_s(nullptr, _wtext, strlen(_text) + 1, _text, _TRUNCATE);
		...
		//BlendStateを引き継いでBegin
		spriteBatch->Begin(DirectX::SpriteSortMode_Deferred, oldBlendState);
		spriteFont->DrawString(spriteBatch, _wtext, Math::Vector2(_x + 640, -_y + 360), _color, 0.0f, { 0,0 }, fontRatio);
		spriteBatch->End();
