...
public:
    ...
    //SE
    void SE(SE_ID id,		//SEのID
	    	float vol		//ボリューム
    );
    //BGM
    //最初から再生
    void BGMPlay(BGM_ID id,		//BGMのID
	    float vol,				//ボリューム
    	bool Loop = false		//ループ再生するか(するならtrue)
    );
    //音量変更
    void SetVolume(BGM_ID id, float vol);
		
    //停止
    void Stop(BGM_ID id);

    //一時停止
    void Pause(BGM_ID id);

    //途中から再生
    void ReStart(BGM_ID id);
...
private:
    ...
    //SEリスト
    std::vector< std::shared_ptr<KdSoundEffect>> se;
    std::vector< std::shared_ptr<KdSoundInstance>> m_seList;
    //BGMリスト
	std::vector< std::shared_ptr<KdSoundEffect>> bgm;
    std::vector< std::shared_ptr<KdSoundInstance>> m_bgmList;
